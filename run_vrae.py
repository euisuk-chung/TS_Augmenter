...
if args.is_generate_train:
    ...
    train_org.to_csv(f'./gen_data/train/original_{args.scale_type}_un_{args.undo}.csv')
    print('>> SAVED TRAIN ORIGINAL Data!! (Loc: gen_data)')
    
    # save reconstructed data
    train_gen = pd.DataFrame(train_recon, columns= cols)
    train_gen.to_csv(f'./gen_data/train/VRAE_{args.scale_type}_un_{args.undo}_hidden_{args.hidden_layer_depth}_win_{args.sequence_length}_ep_{args.n_epochs}.csv')
    print('>> SAVED TRAIN RECONSTRUCTED Data!! (Loc: gen_data)')
...
if args.is_generate_test:
    ...
    test_org.to_csv(f'./gen_data/test/original_{args.scale_type}_un_{args.undo}.csv')
    print('>> SAVED TEST ORIGINAL Data!! (Loc: gen_data)')
    
    # save reconstructed data
    test_gen = pd.DataFrame(test_recon, columns= cols)
    test_gen.to_csv(f'./gen_data/test/VRAE_{args.scale_type}_un_{args.undo}_hidden_{args.hidden_layer_depth}_win_{args.sequence_length}_ep_{args.n_epochs}.csv')
    print('>> SAVED TEST RECONSTRUCTED Data!! (Loc: gen_data)')
