...
def timegan_trainer(model, dataset, args):
    ...
    torch.save(args, f"{args.dload}/args.pickle")
    torch.save(model.state_dict(), f"{args.dload}/model.pt")
    print(f"\nSaved at path: {args.dload}")
...
def timegan_generator(model, Num, args):
    ...
    Args:
        - model (torch.nn.module): The model model that generates synthetic data
        - Num (List[int]): Number of Data
        - args (dict): The model/training configurations
    Returns:
        - generated_data (np.ndarray): The synthetic data generated by the model
    """
    # Load model for inference
    if not os.path.exists(args.dload):
        raise ValueError(f"Model directory not found...")

    # Load arguments and model
    with open(f"{args.dload}/args.pickle", "rb") as fb:
        args = torch.load(fb)

    model.load_state_dict(torch.load(f"{args.dload}/model.pt"))
